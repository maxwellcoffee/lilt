...
After that first tap the walk runs itself. Open **Mix** on the start screen or during a walk. Sliders cover volume, the pad bed, voice, drums, how busy the hats get, how strongly head motion steers, how short hums get chopped, how easily hums catch, echo, brightness, swing, and whether tempo follows your gait or stays locked. Mix is grouped into Sound, Voice, and Move. Street / Room / Soft / Pocket are starting points. Pocket is for a phone in a jacket, with head-steer turned down and the mic catching steps. Mix is optional. End walk lives in that same drawer. Press `M` to open Mix. On a laptop, move the pointer to steer and press Space for a step. Locking tempo shows a Click slider so you can hear the grid without walking.
