...
export class LiltEngine {
  ...
  private hitKick(time: number, gain: number): void {
    ...
    amp.gain.exponentialRampToValueAtTime(0.001, time + 0.32);
    osc.connect(amp);
    ...
    punch.type = "triangle";
    punch.frequency.setValueAtTime(92, time);
    punch.frequency.exponentialRampToValueAtTime(48, time + 0.08);
    punchAmp.gain.setValueAtTime(gain * 0.35, time);
    ...
    clickAmp.gain.setValueAtTime(gain * 0.07, time);
